...
params["lattice_constant"] = params["trap_sep"] + (
    2 * params["particle_radius"] + 1 * ureg.um
) * np.sqrt(2)
params["size"] = 30
...
params["m"] = (
    np.pi
    * (2 * params["particle_radius"]) ** 3
    * params["particle_susceptibility"]
    * params["max_field"]
    / 6
    / params["mu0"]
)
...
params["freedom"] = (
    -(params["mu0"] * params["m"] ** 2 / 4 / np.pi)
    .to(ureg.pN * ureg.nm * ureg.um**3)
    .magnitude
)
